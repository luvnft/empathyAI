...
export default function EmpathyAI() {
    const [chats, setChats] = useState([{type : 'ai', message : `Hi there, I'm EmpathyAI. I'm here to listen and support you. Feel free to share whatever’s on your mind, and we’ll work through it together.`}]);
    const [prompt, setPrompt] = useState("");

    useEffect(() => {
        const fetchAIResponse = async () => {
            if (prompt.trim() !== "") {
                setChats(prevChats => [...prevChats, { type: 'user', message: prompt }]);
                ...
                try {
                    ...
                    if (aiResponse && aiResponse.choices && aiResponse.choices.length > 0) {
                        ...
                        if (aiMessage && aiMessage.message && aiMessage.message.content) {
                            setChats(prevChats => [ ...prevChats, { type: 'ai', message: aiMessage.message.content } ]);
                        } else {
